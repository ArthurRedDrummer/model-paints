<template>
	<section v-show="active && colors.length">
		...
		<ul class="flex flex-col flex-wrap gap-y-2">
			<li class="flex justify-items-start items-center border-b pb-2 gap-x-4" v-for="color in colors" :key="colorKey(color)">
				<h3 class="text-m font-semibold" v-text="color.name"/>
				<p class="text-m" v-text="color.number"/>
				<p class="text-m text-gray-400" v-text="color.factory"/>
				<button class="mr-0 ml-auto" @click.prevent="removeColor(color)">
